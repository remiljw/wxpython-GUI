...
class Mp3Panel(wx.Panel):
    def __init__(self, parent):
        ...
        self.list_ctrl.InsertColumn(3, 'Album Artist', width=140)
        self.list_ctrl.InsertColumn(4, 'Track Number', width=140)
    ...
    def update_mp3_listing(self, folder_path):
        ...
        self.list_ctrl.InsertColumn(1, 'Album', width=140)
        self.list_ctrl.InsertColumn(2, 'Title', width=140)
        self.list_ctrl.InsertColumn(3, 'Album Artist', width=100)
        self.list_ctrl.InsertColumn(4, 'Track Number', width=100)
        ...
        for mp3 in mp3s:
            ...
            print(mp3_object.tag.album_artist)
            # self.list_ctrl.SetItem(index, 3,
            #     mp3_object.tag.album_artist)
            result = mp3_object.tag.track_num
            number, numb = result
            track_num = str(number)
...
class Mp3Frame(wx.Frame):
    def __init__(self):
        super().__init__(parent=None, title='Mp3 Tag Editor', size= (900, 700), style=wx.MINIMIZE_BOX | 
...
class EditDialog(wx.Dialog):
    def __init__(self, mp3):
        ...
        self.album_artist = wx.TextCtrl(
            self, value=self.mp3.tag.album_artist
        )
        self.add_widgets('Album Artist', self.album_artist)
        result = mp3.tag.track_num
        number, numb = result
        track_num = str(number)
        self.track_num = wx.TextCtrl(
            self, value=track_num
        )
        self.add_widgets('Track Number', self.track_num)
    ...
    def on_save(self, event):
        ...
        self.mp3.tag.album_artist = self.album_artist.GetValue()
        self.mp3.tag.track_num = self.track_num.GetValue()
        self.mp3.tag.save()
        self.Close()
